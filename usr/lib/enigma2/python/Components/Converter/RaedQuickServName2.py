...
class RaedQuickServName2(Converter, object):
						...
						if not servicelist is None:
							while True:
								...
								if service == ref:
									...
									name = info and info.getName(provider) or "Unknown"
									return name
		return "N/A"
	...
	def getTransponderInfo(self, info, ref, fmt):
		...
		if self.tpdata is None:
			self.tpdata = ref and info.getInfoObject(ref, iServiceInformation.sTransponderData) or info.getInfoObject(iServiceInformation.sTransponderData)
		...
		if fmt == '' or fmt == '%T':
			if type == 'DVB-C':
				...
				fmt = '%t %F %h %m %g %c'	#(type frequency code_rate_hp transmission_mode guard_interval constellation)
				 ...
				 fmt = '%F%p %Y %f'		#(frequency polarization symbol_rate fec)
		while True:
			pos = fmt.find('%')
			if pos == -1:
				result += fmt
